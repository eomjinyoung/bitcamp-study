...
public class SecurityConfig {
  ...
  public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
    ...
    return http
            ...
            // 4) CSRF(Cross-Site Request Forgery) 기능 설정
            // - CSRF 토큰을 쿠키(XSRF-TOKEN)로 보내도록 설정한다.
            // - 클라이언트로부터 POST, PUT, PATCH, DELETE 요청을 받으면 CSRF 토큰 값을 검증한다.
            // - CSRF 토큰은 요청 헤더(X-XSRF-TOKEN)나 요청 파라미터(_csrf)로 서버에 전송한다.
            .csrf()
